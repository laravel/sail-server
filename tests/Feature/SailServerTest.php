...
class SailServerTest extends TestCase
{
    ...
    public function test_it_removes_duplicated_valid_services()
    {
        ...
        $response->assertStatus(200);
        $response->assertSee('bash -c "laravel new example-app --no-interaction && cd example-app && composer show laravel/sail 2>/dev/null || composer require laravel/sail --dev && php ./artisan sail:install --with=redis "', false);
    }
    ...
    public function test_it_adds_the_devcontainer_upon_request()
    {
        ...
        $response->assertStatus(200);
        $response->assertSee('bash -c "laravel new example-app --no-interaction && cd example-app && composer show laravel/sail 2>/dev/null || composer require laravel/sail --dev && php ./artisan sail:install --with=pgsql --devcontainer"', false);
    }
    ...
    public function test_it_does_not_accept_empty_with_query_when_present()
    {
        ...
        $response->assertStatus(400);
        $response->assertSee('Invalid service name. Please provide one or more of the supported services (mysql, pgsql, mariadb, mongodb, redis, valkey, memcached, meilisearch, typesense, minio, rustfs, mailpit, mailtrap-local, rabbitmq, selenium, soketi) or "none".', false);
    }
